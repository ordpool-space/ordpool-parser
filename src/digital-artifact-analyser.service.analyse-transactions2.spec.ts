...
describe('DigitalArtifactAnalyserService.analyseTransactions', () => {
  ...
  it('should correctly analyze transaction artifacts and count amounts', () => {
    ...
    expect(result.amount.inscription).toBe(2);
    expect(result.amount.runeMint).toBe(2);
    expect(result.amount.brc20Mint).toBe(1);
    expect(result.amount.src20Mint).toBe(1);

    expect(result.fees.inscriptionMints).toBe(3000); // Fees from tx1 and tx2
    expect(result.fees.runeMints).toBe(3500); // Fees from tx2 and tx3
    expect(result.fees.nonUncommonRuneMints).toBe(1500); // Only from tx3 (non-Uncommon mint)
    expect(result.fees.brc20Mints).toBe(2000); // Fees from tx2
    expect(result.fees.src20Mints).toBe(1500); // Fees from tx3
  });
  ...
  it('should track the most active mint tickers for runes, BRC-20, and SRC-20', () => {
    ...
    expect(result.rune.mostActiveMint).toBe('1-0');
    expect(result.rune.mostActiveNonUncommonMint).toBe('1000-2');
